...
sub_df = pd.read_csv("./Submission.csv", names=["Courier_id", "Addr", "Arrival_time", "Departure", "Amount", "Order_id"])

SPEED = 15*1000.0/60
site_df = pd.read_csv("./data/siteData.csv")
spot_df = pd.read_csv("./data/spotData.csv")
shop_df = pd.read_csv("./data/shopData.csv")
ds_order_df = pd.read_csv("./data/ecOrderData.csv")
o2o_order_df = pd.read_csv("./data/o2oOrderData.csv")
courier_df = pd.read_csv("./data/courierData.csv")
node_lng_dict = {}
node_lat_dict = {}
...
def verify9(arrange):
    ...
    for i in range(1, len(mix_arrange_df)):
        ...
        if cur_addr != cmp_e.Addr or amount != cmp_e.Amount or arrival_time != cmp_e.Arrival_time or departure != cmp_e.Departure:
            print "arrange %d" % i,arrange.iloc[0].Courier_id
            return False  
        pre_addr = cur_addr
