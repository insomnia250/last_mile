...
points = {}
csvfile = open('./data/s2/siteData.csv','rU')
...
csvfile.close()
csvfile = file('./data/s2/spotData.csv','rU')
lines = csv.reader(csvfile)
for line in lines:
	sInd,lng,lat = line
	if lng=='Lng':
	...
	points[sInd].extend([float(lng),float(lat)])
csvfile.close()
csvfile = file('./data/s2/shopData.csv','rU')
lines = csv.reader(csvfile)
for line in lines:
	sInd,lng,lat = line
	if lng=='Lng':
...
csvfile.close()
orders={}
csvfile = file('./data/s2/ecOrderData.csv','rU')
lines = csv.reader(csvfile)
for line in lines:
...
csvfile.close()
csvfile = file('./data/s2/o2oOrderData.csv','rU')
lines = csv.reader(csvfile)
for line in lines:
...
# ------------- loading solution ------------
solution = {}
csvfile = file('./Submission_s2_9.csv','rU') # please input your solution path
lines = csv.reader(csvfile)
for line in lines:
	...
	if rNum%2 != 0:
		...
		for rInd in solution[dInd]:
			...
			if penalty_serv>0 or penalty_rout>0:
				tmp_cheat+=1
				if tmp_cheat>0:
					print 'error: do not cheat!',dInd,penalty_serv,penalty_rout,nInd,nInd_befr,time_dist,nInd[1]-nInd_befr[2]-time_dist
