function [splitedTSPSolution , newVehicleNum]= VRP_Split_Route(VRPsolution , O2OshopVec , ecOrderNum)
splitedTSPSolution = [];
...
for i =1 : oriVehicleNum
    route = VRPsolution(i , :);
    Site = route(1);
    if sum(ismember(O2OshopVec , route))==0 && any(route>Site) %如果该快递员任务中不含有O2O,且不是空路
    ...
    elseif   any(route > ecOrderNum+2)%如果有O2O任务 且不是空路
        newVehicleNum = newVehicleNum + 1;
        newSplitRoute = route(route >2);  %表示实际spot 的点
